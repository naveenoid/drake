...
/// sense of the pick-and-place demo are systems that can be used to independently
...
/// high level planners and the implementation level logic of how long an action needs
/// to run, or what kind of LCM message an action must be encoded in etc.
/// Each `ActionPrimitive` can maintain internal state as well as timing information
/// that are task specific. Every `ActionPrimitive` has some default features, as
...
class ActionPrimitive: public systems::LeafSystem<double> {
 public:
  ...
  /// @param desired_update_interval The update period for the unrestricted state
  /// update.
