...
IiwaMove::IiwaMove(
    const RigidBodyTree<double>& iiwa, double desired_update_interval) :
...
IiwaMove::ExtendedAllocateOutputAbstract(
  ...
  if (descriptor.get_index() == output_port_plan_) {
    return_value = systems::AbstractValue::Make<robot_plan_t>(
        robot_plan_t());
  }
  return(return_value);
}
...
void IiwaMove::DoExtendedCalcUnrestrictedUpdate(
  ...
  // Get the current input, check current state, overwrite state if state time_now exceeded.
  ...
  const IiwaActionInput& input_plan = this->EvalAbstractInput(
      context, input_port_primitive_)->GetValue<IiwaActionInput>();
    ...
    case ActionPrimitiveState::WAITING :
      ...
      // A new plan is considered to be input if it has a size greater than 0
       if(input_plan.is_valid && input_plan.time.size() > 0 &&
        ...
        MatrixX<double> q_mat(input_plan.q.front().size(),
                              plan_num_points);
        for (size_t i = 0; i < plan_num_points; ++i) q_mat.col(i) = input_plan.q[i];
        iiwa_action_state.plan =
            EncodeKeyFrames(iiwa_tree_, input_plan.time, info, q_mat);
         // Revist next line for "long" term plans.
        iiwa_action_state.plan_duration = input_plan.time.back() - input_plan.time.front();
...
void IiwaMove::DoExtendedCalcOutput(
  ...
  // Do the output copy.
  robot_plan_t& robot_plan_output = output->GetMutableData(
          output_port_plan_)->GetMutableValue<robot_plan_t>();
