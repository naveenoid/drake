...
class IiwaMove: public ActionPrimitive {
 public:
  ...
  const systems::OutputPortDescriptor<double>& get_robot_plan_output_port() const {
    return this->get_output_port(output_port_plan_);
  }

  virtual std::vector<std::unique_ptr<systems::AbstractValue>> AllocateExtendedAbstractState()
  const final;

  virtual std::unique_ptr<systems::AbstractValue> ExtendedAllocateOutputAbstract(
