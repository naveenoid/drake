...
GripperAction::ExtendedAllocateOutputAbstract(
  ...
  if (descriptor.get_index() == plan_output_port) {
    ...
    return_value = systems::AbstractValue::Make<lcmt_schunk_wsg_command>(default_command);
  }
  return return_value;
}

void GripperAction::DoExtendedCalcUnrestrictedUpdate(
    const systems::Context<double> &context, systems::State<double> *state) const {
  // Gets the current input, check current state, overwrite state if state time exceeded.
  ...
  const GripperActionInput& input_plan = this->EvalAbstractInput(
      context, input_port_primitive_)->GetValue<GripperActionInput>();

  // check current state
  switch(primitive_state) {
    case ActionPrimitiveState::RUNNING :
      if(time - wsg_action_state.start_time >
          wsg_action_state.plan_duration) {
...
void GripperAction::DoExtendedCalcOutput(
  ...
  lcmt_schunk_wsg_command& wsg_plan_output = output->GetMutableData(plan_output_port)
