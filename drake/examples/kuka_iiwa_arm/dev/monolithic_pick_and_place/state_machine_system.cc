...
PickAndPlaceStateMachineSystem::PickAndPlaceStateMachineSystem(
    ...
    input_port_wsg_action_status_ =
        this->DeclareAbstractInputPort().get_index();
    input_port_iiwa_action_status_ =
        this->DeclareAbstractInputPort().get_index();
...
PickAndPlaceStateMachineSystem::AllocateOutputAbstract(
  ...
  if (descriptor.get_index() == output_port_iiwa_action_) {
    return_val = systems::AbstractValue::Make<IiwaActionInput>(
        IiwaActionInput());
...
void PickAndPlaceStateMachineSystem::DoCalcOutput(
  ...
  IiwaActionInput& iiwa_primitive_input = output->GetMutableData(
      output_port_iiwa_action_)->GetMutableValue<IiwaActionInput>();

  GripperActionInput& wsg_primitive_input = output->GetMutableData(
      output_port_wsg_action_)->GetMutableValue<GripperActionInput>();
...
void PickAndPlaceStateMachineSystem::DoCalcUnrestrictedUpdate(
  ...
  const robot_state_t& iiwa_state = this->EvalAbstractInput(
      context, input_port_iiwa_state_)->GetValue<robot_state_t>();
  const robot_state_t& box_state = this->EvalAbstractInput(
      context, input_port_box_state_)->GetValue<robot_state_t>();
  const lcmt_schunk_wsg_status& wsg_status = this->EvalAbstractInput(
      context, input_port_wsg_status_)->GetValue<lcmt_schunk_wsg_status>();
  const ActionPrimitiveState& iiwa_primitive_state = this->EvalAbstractInput(
      context, input_port_iiwa_action_status_)->GetValue<ActionPrimitiveState>();
  const ActionPrimitiveState& wsg_primitive_state = this->EvalAbstractInput(
      context, input_port_wsg_action_status_)->GetValue<ActionPrimitiveState>();
    ...
    switch (internal_state.pick_and_place_state) {
      case PickAndPlaceState::OPEN_GRIPPER:
        ...
        if (!internal_state.wsg_action_initiated) {
          drake::log()->info("StateMachine : OPEN_GRIPPER at {}", context.get_time());
          // Perform wsg action.
      ...
      case PickAndPlaceState::APPROACH_PICK_PREGRASP :
        ...
        if (!internal_state.iiwa_action_initiated) {
          ...
          drake::log()->info("StateMachine : APPROACH_PICK_PREGRASP at {}", context.get_time());
      ...
      case PickAndPlaceState::APPROACH_PICK :
        if (!internal_state.iiwa_action_initiated) {
          ...
          drake::log()->info("StateMachine : APPROACH_PICK at {}", context.get_time());
      ...
      case PickAndPlaceState::LIFT_FROM_PICK :
        ...
        if (!internal_state.iiwa_action_initiated) {
          ...
          drake::log()->info("StateMachine : LIFT_FROM_PICK at {}", context.get_time());
          X_WEndEffector0 = X_WEndEffector1;
          X_WEndEffector1.translation()[2] += kPreGraspHeightOffset;
          ...
          bool res = pick_and_place_demo::PlanStraightLineMotion(
              ...
              X_WEndEffector1, kTightPosTol, kTightRotTol, planner_.get(), &ik_res, &times);
          DRAKE_DEMAND(res);
      ...
      case PickAndPlaceState::APPROACH_PLACE_PREGRASP :
        ...
        if (!internal_state.iiwa_action_initiated) {
          drake::log()->info("StateMachine : APPROACH_PLACE_PREGRASP at {}", context.get_time());
          ...
          if (table == 0) {
            // Table 0 -> table 1.
            X_IiwaObj_desired.translation() = pick_and_place_demo::kPlacePosition1;
            X_IiwaObj_desired.linear() = Matrix3<double>(
                AngleAxis<double>(M_PI / 2., Vector3<double>::UnitZ()));
          } else {
            // Table 1 -> table 0.
            X_IiwaObj_desired.translation() = pick_and_place_demo::kPlacePosition0;
          ...
          bool res = pick_and_place_demo::PlanStraightLineMotion(
              ...
              X_WEndEffector1, kLoosePosTol, kLooseRotTol, planner_.get(), &ik_res,
              &times);
      ...
      case PickAndPlaceState::APPROACH_PLACE :
        if (!internal_state.iiwa_action_initiated) {
          drake::log()->info("StateMachine : APPROACH_PLACE at {}", context.get_time());
          ...
          bool res = pick_and_place_demo::PlanStraightLineMotion(
              world_state_->get_iiwa_q(), 3, 1, X_WEndEffector0,
              X_WEndEffector1, kTightPosTol, kTightRotTol, planner_.get(), &ik_res,
              &times);
          DRAKE_DEMAND(res);
      ...
      case PickAndPlaceState::LIFT_FROM_PLACE :
        if (!internal_state.iiwa_action_initiated) {
          drake::log()->info("StateMachine : LIFT_FROM_PLACE at {}", context.get_time());
      ...
      case PickAndPlaceState::DONE :
        if (!internal_state.iiwa_action_initiated) {
          ...
          internal_state.iiwa_action_initiated = false;
          internal_state.pick_and_place_state =
              PickAndPlaceState::OPEN_GRIPPER;
        }
        break;
    }
  }
