...
StateMachineAndPrimitives<T>::StateMachineAndPrimitives(
  ...
  iiwa_move_ = builder.template AddSystem<IiwaMove>(
      iiwa_tree, iiwa_action_primitive_rate);
  iiwa_move_->set_name("IiwaMove");

  gripper_action_ = builder.template AddSystem<GripperAction>(
      wsg_action_primitive_rate);
  gripper_action_->set_name("GripperAction");

  Isometry3<T> iiwa_base =  Isometry3<T>::Identity();
  iiwa_base.translation() = kRobotBase;

  pick_and_place_state_machine_ = builder.template
      AddSystem<PickAndPlaceStateMachineSystem>(iiwa_base);
  pick_and_place_state_machine_->set_name("PickAndPlaceStateMachine");

  input_port_iiwa_robot_state_t_ =
      builder.ExportInput(
          pick_and_place_state_machine_->get_input_port_iiwa_state());

  input_port_box_robot_state_t_ =
      builder.ExportInput(
  ...
  output_port_iiwa_command_ = builder.ExportOutput(
      iiwa_move_->get_robot_plan_output_port());
  output_port_wsg_command_ = builder.ExportOutput(
      gripper_action_->get_robot_plan_output_port());
